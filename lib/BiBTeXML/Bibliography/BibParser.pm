...
sub readEntry {
    ...
    return BiBTeXML::Bibliography::BibEntry->new( $type, [@tags],
        [ $fn, $sr, $sc, $er, $ec ] );
}
...
sub readTag {
    ...
    my $fn = $reader->getFilename;
    return BiBTeXML::Bibliography::BibTag->new( $name, [@content],
        [ ( $fn, $sr, $sc, $er, $ec ) ] );
}
...
sub readLiteral {
    ...
    my $fn = $reader->getFilename;
    return BiBTeXML::Bibliography::BibString->new( 'LITERAL', $keyword,
        [ ( $fn, $sr, $sc, $er, $ec ) ] );
}
...
sub readBrace {
    ...
    my $fn = $reader->getFilename;
    return BiBTeXML::Bibliography::BibString->new( 'BRACKET', $result,
        [ ( $fn, $sr, $sc, $line, $col + 1 ) ] );
}
...
sub readQuote {
    ...
    # we can add a +1 here, because we did not read a \n
    my $fn = $reader->getFilename;
    return BiBTeXML::Bibliography::BibString->new( 'QUOTE', $result,
        [ ( $fn, $sr, $sc, $line, $col + 1 ) ] );
}
