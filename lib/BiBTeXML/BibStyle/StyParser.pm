...
sub readCommand {
    ...
    return BiBTeXML::BibStyle::StyCommand->new( $name, [@arguments],
        [ ( $fn, $sr, $sc, $er, $ec ) ] );
}
...
sub readBlock {
    ...
    return BiBTeXML::BibStyle::StyString->new( 'BLOCK', [@values],
        [ ( $fn, $sr, $sc, $er, $ec + 1 ) ] );
}
...
sub readNumber {
    ...
    my $fn = $reader->getFilename;
    return BiBTeXML::BibStyle::StyString->new(
        'NUMBER',
        ( $sign . $literal ) + 0,
        [ ( $fn, $sr, $sc, $er, $ec ) ]
    );
...
sub readReference {
    ...
    my $fn = $reader->getFilename;
    return BiBTeXML::BibStyle::StyString->new( 'REFERENCE', $reference,
        [ ( $fn, $sr, $sc, $er, $ec ) ] );
}
...
sub readLiteral {
    ...
    my $fn = $reader->getFilename;
    return BiBTeXML::BibStyle::StyString->new( 'LITERAL', $literal,
        [ ( $fn, $sr, $sc, $er, $ec ) ] );
}
...
sub readQuote {
    ...
    my $fn = $reader->getFilename;
    return BiBTeXML::BibStyle::StyString->new( 'QUOTE', $result,
        [ ( $fn, $sr, $sc, $line, $col + 1 ) ] );
}
